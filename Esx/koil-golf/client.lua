...
Citizen.CreateThread(function()
	...
	while true do
		...
		if distance < 1.5 then
			...
			if golfplaying then
				DisplayHelpText("Appuyez sur ~g~~INPUT_CONTEXT~~s~ pour terminer.")
			else
				DisplayHelpText("Appuyez sur ~g~~INPUT_CONTEXT~~s~ pour commencer (100$).")
			end
			if (IsControlJustReleased(1, 38)) then
				...
				if (golfplaying) then
					...
					TriggerEvent("customNotification","Appuyez sur E pour tirer, A-D pour tourner, Y pour la puissance")
				end
			end
	...
	while golfstate == 1 do
		...
		if GetVehiclePedIsIn(GetPlayerPed(-1), false) == 0 then
			...
			if distance < 50.0 then
				DisplayHelpText("Ramener la balle, appuyez sur ~g~E~s~")
				if ( IsControlJustReleased(1, 38) ) then
					dropShot()
				end
